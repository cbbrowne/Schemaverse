...
-- Seize planets
update planets set name = 'FunbusterLand' where id in (select planet from planets_in_range) and conqueror_id = my_player;
...
>>>>>>> fc9bfac15bf1a6b58996715819ed3de96adcbe46
...
if (select fuel_reserve from my_player) > 5000 then
   ...
   insert into want_ships (fleet_id, name, attack, defense, engineering, prospecting,location_x,location_y)
      select miner_fleet, 'Miner', 0,2,2,16, p.location_x, p.location_y from t_planets p where conqueror_id = my_player
      order by random() limit 1;
   ...
   if random() > 0.8 then
     ...
     insert into want_ships (fleet_id, name, attack, defense, engineering, prospecting,location_x,location_y)
       select miner_fleet, 'Miner', 4,2,1,13, p.location_x, p.location_y from t_planets p where conqueror_id = my_player
       order by random() limit 1;
   end if;
